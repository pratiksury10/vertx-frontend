...
const RevenueSelected = () => {
  ...
  //   navigate('/industry', { state: { selectedRevenue } });
  // };
  ...
  const handleContinue = async () => {
    ...
    try {
      await axios.post('http://localhost:5009/api/startup', { userId: 'your-unique-user-id',stage: selectedStage, location: selectedCountry, raise: selectedAmount, revenue: selectedRevenue });
      navigate('/industry', { state: { selectedStage, selectedCountry, selectedAmount, selectedRevenue } });
    } catch (error) {
  ...
  return (
       ...
       <div className="w-[1580px] h-[832px] bg-gradient-to-b from-[#0F0C29] via-[#08080d] to-[#0b0b3f] text-white flex flex-col items-center justify-center p-6">
          ...
          <div className="absolute w-[572px] h-[54px] top-[170px] left-[340px] mr-80 text-white space-y-3">
            ...
            {/* Back Button */}
                <button className="relative w-[33px] h-[17px] font-inter font-normal text-[14px] leading-[100%] text-white" onClick={() => navigate("/revenue")}>
                   Back
                </button>
